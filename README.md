...
- **可主题化 UI Shell**：组件包拥有 DOM 与必要结构样式，主题通过稳定 token 和精选 component/part/state 钩子定义视觉。
...
- 开箱即用：`@dragcraft/themes` 提供完整 Standard 工作台主题，一行 import 获得结构与视觉样式。
...
```typescript
// 1. 导入工作台主题（二选一）
import '@dragcraft/themes'              // 默认 Standard：紧凑、企业蓝调的产品工作台
...
- 高级主题：导入 `@dragcraft/themes/structure`，再基于公开主题契约实现完整视觉配方。
