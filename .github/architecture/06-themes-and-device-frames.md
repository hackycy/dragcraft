...
Frame 的 flow、chrome 和 layer 横向内容边界都位于设备边框内沿，因此 369px 物料不会被 375px Frame 的边框遮挡；这个边界只表达真实设备结构，不承担 selection 补偿。Frame 不再为选中态增加 padding、offset 或 gutter，只继续处理真实 safe area 与 chrome reserve。root plane 固定覆盖完整 Frame；content plane 位于 scroller surface 并随内容滚动；viewport plane 固定在设备 viewport 内。root flow 投影监听祖先滚动以跟随物料，最终仍由各平面的 `overflow: hidden` 自然裁剪。
...
- 通过 `selectionPresentation` 注册覆盖完整 Frame 的 root 平面、随 scroller 的 content 平面和固定在 viewport 的 viewport 平面；Frame 负责设备边框内沿的内容边界、真实 safe area/chrome reserve、边框结构层、层级和裁剪，不读取 selected 状态，也不为交互视觉增加布局留白。
...
Device frame 的内容区由 `useFrameViewport()` 渲染为 viewport、滚动容器、内容 surface 和两个 selection presentation plane。具体 DOM 层级与 `.dc-device-frame*` class 是 `@dragcraft/device-frames` 的私有实现，不属于工作台主题契约。
...
`DeviceFrameShell` 接收 renderer 传入的 `schema`，把 `schema.root.style.surface` 作为开放样式对象传给具体 frame。最终样式应用到声明了 `data-dc-component="container-shell"` 的内容 surface，用于预览页面承载面效果。
...
- 所有 Frame 组件内容 surface 必须声明 `data-dc-component="container-shell"`，以复用 Renderer 结构契约。
