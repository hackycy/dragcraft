...
Renderer 固定 selection projection 的位置和尺寸。默认 `DefaultNodeSelection` 在其中绘制完整连续矩形，工作台主题通过 `--dc-node-selection-stroke-width`、`--dc-color-accent` 和 `node-selection` 的公开 part/state 提供视觉；`--dc-node-selection-root-block-overlap` 与 `--dc-node-selection-root-inline-overlap` 由 Frame 参与结构协作。业务可以替换 `rendererExtensions.nodeSelection`，但不能借 presenter 改写投影范围、平面路由或 Frame 裁剪。
