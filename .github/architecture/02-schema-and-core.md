...
`SchemaStore` 是 Core 内部可写模块，基于 `shallowRef`、`ref` 和 `triggerRef` 管理响应式状态。`SchemaStoreInstance` 与 `createSchemaStore()` 不从包根导出。
...
- 不暴露 `getRawSchema()`、`setSchema()`、`triggerUpdate()` 或 transient patch。所有 schema 写入仍必须进入 `engine.execute()`。

### EngineState

`engine.state` 是公开读取 facade：

- `getSchema()`：返回当前 schema 的深拷贝并递归冻结；类型为 `DeepReadonly<DesignerSchema>`。
- `getNodeById(id)`：返回同一类深冻结节点快照。
