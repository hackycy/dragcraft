...
describe('useCanvasPan', () => {
  ...
  it('applies pan and pixel snap as independent stage translations', () => {
    ...
    const boundaryRule = css.match(/\.dc-canvas__content > \.dc-application-surface > \[data-dc-toolbar-boundary\]\s*\{[^}]*\}/)?.[0]
    const boundedContentRule = css.match(/\.dc-canvas__content--bounded\s*\{[^}]*\}/)?.[0]
    const boundedRootRule = css.match(/\.dc-canvas__content--bounded > \.dc-application-surface\s*\{[^}]*\}/)?.[0]
    const rootRule = css.match(/\.dc-canvas__content > \.dc-application-surface\s*\{[^}]*\}/)?.[0]
