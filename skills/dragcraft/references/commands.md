...
读取 [commands resources](resources/commands.json)，再检查当前版本的 Schema `AuthoringAction`、Presentation 语义 action、结果、history 和状态观察方式。
...
3. 分别处理 `committed`、`unchanged`、`rejected` 和 `confirmation-required`。只有实际产生 Schema 变化的 action 进入 history；宿主需要观察数据变化时使用 Vue `watch(designer.document)`，不要假定存在公共业务事件。
...
测试同时证明成功、no-op 与拒绝结果的 Schema、history 和 `designer.document` 观察语义；undo/redo 恢复同一事务；失败路径不产生部分写入。
