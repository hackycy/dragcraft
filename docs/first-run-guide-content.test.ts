...
describe('first-run docs content', () => {
  ...
  it('explains where widget materials and field adapters come from', () => {
    ...
    expect(materialsAndFields).toContain('import { createDesigner } from \'@dragcraft/designer\'')
    expect(materialsAndFields).toContain('import { buildComponentMap, getWidgetMetas } from \'@dragcraft/widgets\'')
    expect(materialsAndFields).toContain('import { createAntDesignVueFields } from \'@dragcraft/fields-ant-design-vue\'')
    expect(materialsAndFields).toContain('widgetMetas: getWidgetMetas(myWidgetDefinitions),')
    expect(materialsAndFields).toContain('componentMap: buildComponentMap(myWidgetDefinitions),')
    expect(materialsAndFields).toContain('fieldComponentMap: createAntDesignVueFields(),')
    expect(materialsAndFields).toContain('设计器左侧显示哪些卡片，取决于 `widgetMetas` 里的 `DesignerWidgetMeta.material`。')
  ...
  it('explains when to apply themes alone and when to add device frames', () => {
    ...
    expect(themesAndDeviceFrames).toContain('import { createDesigner } from \'@dragcraft/designer\'')
    expect(themesAndDeviceFrames).toContain('DEVICE_FRAME_CONTEXT_KEY')
    expect(themesAndDeviceFrames).toContain('import { provide } from \'vue\'')
    expect(themesAndDeviceFrames).toContain('const deviceCtx = createDeviceFrameContext({ initialDevice: \'iphone\' })')
    expect(themesAndDeviceFrames).toContain('provide(DEVICE_FRAME_CONTEXT_KEY, deviceCtx)')
  ...
  it('points the root README docs section at the consumer site and maintainer architecture docs', () => {
    ...
    expect(readme).toContain('- [`docs/`](./docs/): 面向接入者的文档站点')
    expect(readme).toContain('- [`.github/architecture/`](./.github/architecture/): 面向维护者的架构文档')
  })
})
