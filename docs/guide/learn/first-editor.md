...
最小编辑器由一个 `MaterialDefinition`、一个字段 adapter map 和一个 `DcDesigner` 组成。完成后，你可以拖入文本、修改属性并撤销修改。
...
`type` 是 Schema 中保存的稳定标识。`presentation.preview` 是设计态组件；组件名可以重构，`type` 改名则需要宿主离线重写 Schema。
...
`materials` 是唯一的物料注册面；字段 adapter 只负责把表单字段键映射为控件。
...
如果左栏存在物料但画布显示 fallback，检查 `presentation` 是否使用了相同的 `type`。如果右栏显示未知字段，检查 `fieldComponentMap` 是否注册了 `inspector.formSchema` 中的组件键。
