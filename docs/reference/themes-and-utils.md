...
`@dragcraft/themes` 负责结构 CSS 聚合与默认工作台视觉，`@dragcraft/utils` 负责跨包复用的纯函数。
...
这段代码说明这两个包并不承担同一种职责。主题包的默认入口与 `/standard` 都加载必要结构 CSS、完整默认 token 和 Standard 基线配方，`/material` 在相同基线上加载 Material 差异；`/structure` 只提供高级主题所需的结构层。`/theme-contract.json` 和 `/css-custom-data.json` 分别提供机器契约与编辑器补全。utils 则提供可以在多个包之间复用的基础能力，比如 i18n、事件分发和短 id。
