...
`/standard`、`/material` 与 `/structure` 都是自包含 CSS 入口。使用者不需要再单独导入 ui、designer、renderer 或 form-generator 的结构样式，也不会依赖 monorepo 内部路径。Standard 与 Material 还包含 `DcScrollArea` 的默认 recipe 和完整 `--dc-scroll-area-*` token。
...
语言包合并和 locale 切换见 [编辑器国际化](/guide/i18n)；Schema 的导入、导出和版本处理见 [Schema 生命周期](/guide/import-export-and-i18n)。
