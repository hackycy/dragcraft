...
export interface ContainerVariantMigrationContext {
  schema: unknown
  container: NodeDefinition
  fromVariantId: string
  toVariantId: string
  fromVariant: ContainerVariantDefinition
  toVariant: ContainerVariantDefinition
  state: { variant: string, regions: Record<string, NodeDefinition[]> }
}

export type ContainerVariantMigrationResult
  = | { allowed: true, state: { variant: string, regions: Record<string, NodeDefinition[]> } }
    | { allowed: false, code: string, details?: Record<string, unknown> }
...
export function isMaterialVisible(meta: MaterialMeta): boolean {
