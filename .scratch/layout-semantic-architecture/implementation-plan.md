...
| G9 Deletion Gate | passed | Legacy Adapter、旧 Engine/Command/Registry/LayoutPlan、Renderer、Widgets 及实现耦合测试已物理删除；Designer 已移除旧 backend dependencies，workspace/lockfile/build graph 不再包含旧 packages，旧 Renderer CSS fixture 已迁移到 Designer-owned Presentation CSS。Core 无 Vue dependency，Designer 不依赖 Renderer/Widgets；旧 package/import active references 清理完成（主题校验脚本仅保留防回归字符串检查）。directed：Designer 83/83、Device Frames 15/15、Playground 17/17；完整：`pnpm build`、`pnpm lint`、`pnpm typecheck`、`pnpm test`（Core 65、Designer 83、Device Frames 15、Form Generator 86、Playground 17、Guide 14 等全 workspace）和 `pnpm test:browser`（Chromium 31/31）通过；三个 Playground 模板人工验收已获用户确认。G9 为最终 Gate，无后续 Gate。 |
