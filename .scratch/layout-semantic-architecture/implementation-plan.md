...
| G9 Deletion Gate | passed | 页面全局样式绑定通过 `page.update` 写入 `page.style.surface`，背景色、背景图、背景尺寸和内边距会驱动画布 surface；最终 Playground visual materials 均提供 `style.container` 与 `style.content` 的 margin/padding 字段（支持负 margin），物料也可用 `useWidgetRuntime()` 注入受控样式更新。directed：Designer 26/26、Playground 5/5；完整：`pnpm build`、`pnpm lint`、`pnpm typecheck`、`pnpm test`（Designer 84、Playground 18、Guide 14 等全 workspace）和 `pnpm test:browser`（Chromium 31/31）通过；三个 Playground 模板人工复验已获用户确认。G9 为最终 Gate，无后续 Gate。 |
