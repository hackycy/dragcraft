...
完成 `.scratch/layout-semantic-architecture/goal-runbook.md` 中启动本次 Goal 时唯一 `active` 的 Gate。开始时锁定该 Gate 的编号；该编号是本次 Goal 不可扩大的执行边界。只完成该 Gate，不得进入下一个 Gate。

先阅读 `CLAUDE.md`、适用的 `AGENTS.md` / `CONTEXT.md`、`.scratch/layout-semantic-architecture/implementation-plan.md`、`.scratch/layout-semantic-architecture/goal-runbook.md`，以及当前 Gate 引用的 decision 文档和相关代码。以这些文件为唯一执行依据。
